...
def season_episodes(id, season_number):
    ...
    for episode in season["episodes"]:
        watching_episode = next(
            (i for i in watching_season["episodes"] if i["number"] == episode["number"]), None
            )
...
def play(id, title, video_info, video_data=None, poster=None):
    ...
    url = get_mlink(
        video_data,
        quality=__addon__.getSetting("video_quality"),
        stream_type=__addon__.getSetting("stream_type"),
        ask_quality=__addon__.getSetting("ask_quality")
    )
    li = ExtendedListItem(
        title,
        path=url,
